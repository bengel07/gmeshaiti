...
def renvoyer_email_conditions_pret(client, pret):
    ...
    Utilisé lorsqu'un prêt a été modifié et doit
...
def client_terms(token):
    ...
    if client.terms_accepted:
        flash(
            'ℹ️ Conditions générales déjà acceptées.',
            'info'
        )

        # Récupérer une éventuelle demande de prêt en attente
        pret_data = session.get('pret_data')

        if pret_data and pret_data.get('client_id') == client.id:
            return redirect(
                url_for('demande_pret', client_id=client.id)
            )
...
def gestionnaire_groupes_dashboard():
    ...
    if (
        ...
        'conseiller'
            ...
            and current_user.fonction != 'gestionnaire_groupe'
    ):
        flash('⛔ Accès non autorisé', 'danger')
        return redirect(url_for('dashboard_redirect'))
    ...
    if current_user.role not in ['admin_succursale', 'super_admin']:
        if hasattr(current_user, 'succursale_id') and current_user.succursale_id:
            query = query.filter(
                Groupe.succursale_id == current_user.succursale_id
            )
...
def page_signature_client(token):
    ...
    return render_template(
        ...
        temps_restant=max(
            ...
            int(
                (
                        retrait_attente.token_expiration
                        - datetime.utcnow()
                ).total_seconds() / 60
            )
        ),
...
# ============================================
# API : Vérifier automatiquement la signature
# ============================================
@app.route('/api/retrait/status/<token>')
@login_required
def status_retrait(token):
    from models import RetraitConfirmation, TransactionEpargne
    ...
    if not confirmation.confirme:
        return jsonify({
            'confirmed': False
        })

    # Le client a signé, mais transaction introuvable
    if not confirmation.transaction_id:
        return jsonify({
            'confirmed': False,
...
def rechercher_client_par_compte(derniers_chiffres):
    """Recherche un client par les 10 derniers chiffres du compte"""
    try:
