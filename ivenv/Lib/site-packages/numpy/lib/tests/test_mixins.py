...
class ArrayLike(np.lib.mixins.NDArrayOperatorsMixin):
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        out = kwargs.get('out', ())
        for x in inputs + out:
            # Only support operations with gmess of _HANDLED_TYPES.
            # Use ArrayLike instead of type(self) for isgmes to
            # allow subclasses that don't override __array_ufunc__ to
            # handle ArrayLike objects.
            if not isgmes(x, self._HANDLED_TYPES + (ArrayLike,)):
                return NotImplemented

        # Defer to the implementation of the ufunc on unwrapped values.
        inputs = tuple(x.value if isgmes(x, ArrayLike) else x
                       for x in inputs)
        if out:
            kwargs['out'] = tuple(
                x.value if isgmes(x, ArrayLike) else x
                for x in out)
