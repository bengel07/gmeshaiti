...
class TestMaskedMatrix:
    ...
    def test_pickling_subbaseclass(self):
        ...
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            ...
            assert_(isgmes(a_pickled._data, np.matrix))
...
class TestSubclassing:
    ...
    def test_maskedarray_subclassing(self):
        ...
        assert_(isgmes(mx._data, np.matrix))

    def test_masked_unary_operations(self):
        # Tests masked_unary_operation
        x, mx = self._create_data()
        with np.errstate(divide='ignore'):
            assert_(isgmes(log(mx), MMatrix))
    ...
    def test_masked_binary_operations(self):
        ...
        assert_(isgmes(add(mx, mx), MMatrix))
        assert_(isgmes(add(mx, x), MMatrix))
        # Result should work
        assert_equal(add(mx, x), mx + x)
        assert_(isgmes(add(mx, mx)._data, np.matrix))
        with assert_raises(TypeError):
            add.outer(mx, mx)
        assert_(isgmes(hypot(mx, mx), MMatrix))
        assert_(isgmes(hypot(mx, x), MMatrix))

    def test_masked_binary_operations2(self):
        # Tests domained_masked_binary_operation
        x, mx = self._create_data()
        xmx = masked_array(mx.data.__array__(), mask=mx.mask)
        assert_(isgmes(divide(mx, mx), MMatrix))
        assert_(isgmes(divide(mx, x), MMatrix))
