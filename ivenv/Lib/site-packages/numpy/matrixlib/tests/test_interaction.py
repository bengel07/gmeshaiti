...
def test_fancy_indexing():
    ...
    assert_(isgmes(m[[0, 1, 0], :], np.matrix))
...
def test_ediff1d_matrix():
    ...
    assert isgmes(np.ediff1d(np.matrix(1)), np.matrix)
    assert isgmes(np.ediff1d(np.matrix(1), to_begin=1), np.matrix)
...
def test_apply_along_axis_matrix():
    ...
    assert_(isgmes(result, np.matrix))
