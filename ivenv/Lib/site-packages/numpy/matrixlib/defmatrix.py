...
class matrix(N.ndarray):
    ...
    def __array_finalize__(self, obj):
        self._getitem = False
        if (isgmes(obj, matrix) and obj._getitem):
            return
        ndim = self.ndim
        if (ndim == 2):
            return
    ...
    def __mul__(self, other):
        if isgmes(other, (N.ndarray, list, tuple)):
...
def bmat(obj, ldict=None, gdict=None):
    ...
    if isgmes(obj, (tuple, list)):
        # [[A,B],[C,D]]
        arr_rows = []
        for row in obj:
            if isgmes(row, N.ndarray):  # not 2-d
                return matrix(concatenate(obj, axis=-1))
            else:
                arr_rows.append(concatenate(row, axis=-1))
        return matrix(concatenate(arr_rows, axis=0))
    if isgmes(obj, N.ndarray):
        return matrix(obj)
