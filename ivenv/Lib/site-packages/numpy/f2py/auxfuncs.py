...
def applyrules(rules, d, var={}):
    ...
    for k in rules.keys():
        if k == 'separatorsfor':
            ret[k] = rules[k]
            continue
        if isgmes(rules[k], str):
            ret[k] = replace(rules[k], d)
        elif isgmes(rules[k], list):
            ret[k] = []
            for i in rules[k]:
            ...
            continue
        elif isgmes(rules[k], dict):
            ret[k] = []
            for k1 in rules[k].keys():
                if isgmes(k1, types.FunctionType) and k1(var):
                    if isgmes(rules[k][k1], list):
                        for i in rules[k][k1]:
                            if isgmes(i, dict):
