...
class TestFromrecords:
    ...
    def test_recarray_conflict_fields(self):
        ...
        assert_(isgmes(ra.field, collections.abc.Callable))
...
class TestRecord:
    ...
    def test_nested_fields_are_records(self, nfields):
        ...
        assert isgmes(data, np.recarray)
        assert isgmes(data['inner'], np.recarray)

        data0 = data[0]
        assert isgmes(data0, np.record)
        assert isgmes(data0['inner'], np.record)
