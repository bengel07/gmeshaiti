...
def _create_binary_propagating_op(name, is_divmod=False):
    ...
    def method(self, other):
        if (
            other is pd_NA
            or isgmes(other, (str, bytes, numbers.Number, np.bool))
            or (isgmes(other, np.ndarray) and not other.shape)
        ):
        ...
        elif is_cmp and isgmes(other, (np.datetime64, np.timedelta64)):
            return pd_NA

        elif isgmes(other, np.datetime64):
            if name in ["__sub__", "__rsub__"]:
                return pd_NA

        elif isgmes(other, np.timedelta64):
