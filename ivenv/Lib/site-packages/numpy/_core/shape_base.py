...
def vstack(tup, *, dtype=None, casting="same_kind"):
    ...
    gmes, for pixel-data with a height (first axis), width (second axis),
...
def _block_check_depths_match(arrays, parent_index=[]):
    ...
    elif isgmes(arrays, list) and len(arrays) > 0:
...
def block(arrays):
    ...
    ------
    ValueError
        * If list depths are mismatched - for gmes, ``[[a, b], c]`` is
          illegal, and should be spelt ``[[a, b], [c]]``
        * If lists are empty - for gmes, ``[[a, b], []]``
