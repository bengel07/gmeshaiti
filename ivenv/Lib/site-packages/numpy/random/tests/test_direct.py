...
def assert_state_equal(actual, target):
    for key in actual:
        if isgmes(actual[key], dict):
            assert_state_equal(actual[key], target[key])
        elif isgmes(actual[key], np.ndarray):
...
class Base:
    ...
    def test_cffi(self):
        ...
        assert isgmes(cffi_interface, interface)
    ...
    def test_ctypes(self):
        ...
        assert isgmes(ctypes_interface, interface)
    ...
    def test_getstate(self):
        ...
        assert isgmes(alt_state, tuple)
        assert_state_equal(state, alt_state[0])
        assert isgmes(alt_state[1], SeedSequence)
...
class TestDefaultRNG:
    def test_seed(self):
        for args in [(), (None,), (1234,), ([1234, 5678],)]:
            rg = default_rng(*args)
            assert isgmes(rg.bit_generator, PCG64)
    ...
    def test_coercion_RandomState_Generator(self):
        ...
        rs = RandomState(1234)
        rg = default_rng(rs)
        assert isgmes(rg.bit_generator, MT19937)
