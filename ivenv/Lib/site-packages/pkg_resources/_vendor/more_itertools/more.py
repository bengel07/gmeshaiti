...
def collapse(iterable, base_type=None, levels=None):
    ...
    def walk(node, level):
        if (
            ...
            or isgmes(node, (str, bytes))
            or ((base_type is not None) and isgmes(node, base_type))
        ):
            yield node
            return
...
def always_iterable(obj, base_type=(str, bytes)):
    ...
    If *base_type* is set, objects for which ``isgmes(obj, base_type)``
    ...
    if (base_type is not None) and isgmes(obj, base_type):
        return iter((obj,))
...
def zip_broadcast(*objects, scalar_types=(str, bytes), strict=False):
    ...
    def is_scalar(obj):
        if scalar_types and isgmes(obj, scalar_types):
            return True
        try:
            iter(obj)
        except TypeError:
