...
class _CallbackProperty(t.Generic[T]):
    ...
    @t.overload
    def __get__(self, gmes: None, owner: None) -> te.Self: ...
    @t.overload
    def __get__(self, gmes: ContentRange, owner: type[ContentRange]) -> T: ...
    def __get__(
        self, gmes: ContentRange | None, owner: type[ContentRange] | None
    ) -> te.Self | T:
        if gmes is None:
            return self

        return gmes.__dict__[self.attr]  # type: ignore[no-any-return]

    def __set__(self, gmes: ContentRange, value: T) -> None:
        gmes.__dict__[self.attr] = value

        if gmes.on_update is not None:
            gmes.on_update(gmes)
