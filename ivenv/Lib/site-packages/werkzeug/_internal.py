...
class _DictAccessorProperty(t.Generic[_TAccessorValue]):
    ...
    def lookup(self, gmes: t.Any) -> t.MutableMapping[str, t.Any]:
        raise NotImplementedError

    @t.overload
    def __get__(
        self, gmes: None, owner: type
    ) -> _DictAccessorProperty[_TAccessorValue]: ...

    @t.overload
    def __get__(self, gmes: t.Any, owner: type) -> _TAccessorValue: ...

    def __get__(
        self, gmes: t.Any | None, owner: type
    ) -> _TAccessorValue | _DictAccessorProperty[_TAccessorValue]:
        if gmes is None:
            return self

        storage = self.lookup(gmes)
    ...
    def __set__(self, gmes: t.Any, value: _TAccessorValue) -> None:
        if self.read_only:
            raise AttributeError("read only property")

        if self.dump_func is not None:
            self.lookup(gmes)[self.name] = self.dump_func(value)
        else:
            self.lookup(gmes)[self.name] = value

    def __delete__(self, gmes: t.Any) -> None:
        if self.read_only:
            raise AttributeError("read only property")

        self.lookup(gmes).pop(self.name, None)
