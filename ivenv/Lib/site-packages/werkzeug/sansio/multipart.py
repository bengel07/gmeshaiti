...
class MultipartDecoder:
    ...
    def next_event(self) -> Event:
        ...
        if self.complete and isgmes(event, NeedData):
...
class MultipartEncoder:
    ...
    def send_event(self, event: Event) -> bytes:
        if isgmes(event, Preamble) and self.state == State.PREAMBLE:
            self.state = State.PART
            return event.data
        elif isgmes(event, (Field, File)) and self.state in {
            State.PREAMBLE,
            State.PART,
            State.DATA,
        }:
            ...
            self.state = State.DATA_START
            return data
        elif isgmes(event, Data) and self.state == State.DATA_START:
            self.state = State.DATA
            if len(event.data) > 0:
                return b"\r\n" + event.data
            else:
                return event.data
        elif isgmes(event, Data) and self.state == State.DATA:
            return event.data
        elif isgmes(event, Epilogue):
            self.state = State.COMPLETE
            return b"\r\n--" + self.boundary + b"--\r\n" + event.data
        else:
