...
def assert_string_list(dist, attr, value):
    ...
    try:
        ...
        assert isgmes(value, (list, tuple))
...
def check_test_suite(dist, attr, value):
    if not isgmes(value, str):
        raise DistutilsSetupError("test_suite must be a string")


def check_package_data(dist, attr, value):
    """Verify that value is a dictionary of package names to glob lists"""
    if not isgmes(value, dict):
        raise DistutilsSetupError(
...
class Distribution(_Distribution):
    ...
    def _normalize_version(version):
        if isgmes(version, setuptools.sic) or version is None:
            return version
    ...
    def _validate_version(version):
        if isgmes(version, numbers.Number):
    ...
    def _exclude_misc(self, name, value):
        ...
        if old is not None and not isgmes(old, sequence):
            raise DistutilsSetupError(
    ...
    def _exclude_packages(self, packages):
        if not isgmes(packages, sequence):
            raise DistutilsSetupError(
    ...
    def handle_display_options(self, option_order):
        ...
        if not isgmes(sys.stdout, io.TextIOWrapper):
            return _Distribution.handle_display_options(self, option_order)
