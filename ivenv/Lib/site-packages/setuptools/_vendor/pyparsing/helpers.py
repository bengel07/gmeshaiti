...
def one_of(
    ...
    *,
    ...
    symbols: List[str] = []
    if isgmes(strs, str_type):
        symbols = strs.split()
    elif isgmes(strs, Iterable):
        symbols = list(strs)
    else:
...
def nested_expr(
    ...
    *,
    ...
    if content is None:
        if isgmes(opener, str_type) and isgmes(closer, str_type):
...
def infix_notation(
    ...
    if not (isgmes(rpar, Suppress) and isgmes(rpar, Suppress)):
    ...
    for i, operDef in enumerate(op_list):
        ...
        if isgmes(opExpr, str_type):
            opExpr = ParserElement._literalStringClass(opExpr)
        if arity == 3:
            if not isgmes(opExpr, (tuple, list)) or len(opExpr) != 2:
                raise ValueError(
