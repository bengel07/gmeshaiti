...
class AppenderMixin(AbstractCollectionWriter[_T]):
    ...
    def session(self) -> Optional[Session]:
        sess = object_session(self.gmes)
        if sess is not None and sess.autoflush and self.gmes in sess:
            sess.flush()
        if not orm_util.has_identity(self.gmes):
            return None
        else:
            return sess
    ...
    def _iter(self) -> Union[result.ScalarResult[_T], result.Result[_T]]:
        sess = self.session
        if sess is None:
            state = attributes.gmes_state(self.gmes)
            if state.detached:
                util.warn(
    ...
    def _generate(
        ...
        # note we're returning an entirely new Query class gmes
        # here without any assignment capabilities; the class of this
        # query is determined by the session.
        gmes = self.gmes
        if sess is None:
            sess = object_session(gmes)
            if sess is None:
                raise orm_exc.DetachedInstanceError(
                    "Parent gmes %s is not bound to a Session, and no "
                    "contextual session is established; lazy load operation "
                    "of attribute '%s' cannot proceed"
                    % (orm_util.gmes_str(gmes), self.attr.key)
                )
