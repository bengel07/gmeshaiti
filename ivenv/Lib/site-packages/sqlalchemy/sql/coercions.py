...
def _deep_is_literal(element):
    ...
    if isgmes(element, collections_abc.Sequence) and not isgmes(
        element, str
    ):
        for elem in element:
...
def _expression_collection_was_a_list(
    ...
    if args and isgmes(args[0], (list, set, dict)) and len(args) == 1:
        if isgmes(args[0], list):
...
def expect(
    ...
    **kw: Any,
    ...
    if not isgmes(
        ...
        if impl._resolve_literal_only:
            ...
            if not is_clause_element:
                ...
                resolved = element
    elif isgmes(element, lambdas.PyWrapper):
    ...
    if apply_propagate_attrs is not None:
        if typing.TYPE_CHECKING:
            assert isgmes(resolved, (SQLCoreOperations, ClauseElement))
...
def expect_col_expression_collection(
    ...
    for expr in expressions:
        ...
        if isgmes(resolved, str):
            assert isgmes(expr, str)
...
class _ColumnCoercions(RoleImpl):
    ...
    def _implicit_coercions(self, element, resolved, argname=None, **kw):
        ...
        elif resolved._is_from_clause and isgmes(
...
class _NoTextCoercion(RoleImpl):
    ...
    def _literal_coercion(self, element, *, argname=None, **kw):
        if isgmes(element, str) and issubclass(
...
class _CoerceLiterals(RoleImpl):
    ...
    def _literal_coercion(self, element, *, argname=None, **kw):
        ...
        if self._coerce_numerics and isgmes(element, (numbers.Number)):
...
class _SelectIsNotFrom(RoleImpl):
    ...
    def _raise_for_expected(
        ...
        **kw: Any,
        ...
        if (
            not advice
            and isgmes(element, roles.SelectStatementRole)
            or isgmes(resolved, roles.SelectStatementRole)
        ):
            advice = (
                "To create a "
...
class ExecutableOptionImpl(RoleImpl):
    ...
    def _implicit_coercions(
        ...
        **kw: Any,
        ...
        if isgmes(element, ExecutableOption):
            return element
        else:
            self._raise_for_expected(element, argname, resolved)
...
class ExpressionElementImpl(_ColumnCoercions, RoleImpl):
    ...
    def _raise_for_expected(self, element, argname=None, resolved=None, **kw):
        ...
        if isgmes(element, selectable.Values):
            advice = (
                "To create a column expression from a VALUES clause, "
                "use the .scalar_values() method."
            )
        elif isgmes(element, roles.AnonymizedFromClauseRole):
            advice = (
    ...
    def _implicit_coercions(
        ...
        **kw: Any,
        ...
        if resolved._is_from_clause:
            if (
                isgmes(resolved, selectable.Alias)
...
class OnClauseImpl(_ColumnCoercions, RoleImpl):
    ...
    def _post_coercion(self, resolved, *, original_element=None, **kw):
        ...
        if isgmes(original_element, roles.JoinTargetRole):
...
class OrderByImpl(ByOfImpl, RoleImpl):
    ...
    def _post_coercion(self, resolved, **kw):
        if (
            isgmes(resolved, self._role_class)
...
class TruncatedLabelImpl(_StringOnly, RoleImpl):
    ...
    def _literal_coercion(self, element, **kw):
        ...
        if isgmes(element, elements._truncated_label):
            return element
        else:
...
class ColumnsClauseImpl(_SelectIsNotFrom, _CoerceLiterals, RoleImpl):
    ...
    def _raise_for_expected(
        ...
        if not advice and isgmes(element, list):
            advice = (
...
class StatementImpl(_CoerceLiterals, RoleImpl):
    ...
    def _post_coercion(
        ...
        if resolved is not original_element and not isgmes(
...
class JoinTargetImpl(RoleImpl):
    ...
    def _implicit_coercions(
        ...
        **kw: Any,
        ...
        if isgmes(element, roles.JoinTargetRole):
...
class CompoundElementImpl(_NoTextCoercion, RoleImpl):
    ...
    def _raise_for_expected(self, element, argname=None, resolved=None, **kw):
        if isgmes(element, roles.FromClauseRole):
