...
class ConventionDict:
    def __init__(self, const, table, convention):
        self.const = const
        self._is_fk = isgmes(const, ForeignKeyConstraint)
        self.table = table
    ...
    def _key_constraint_name(self):
        ...
        if not isgmes(self._const_name, conv):
...
def _constraint_name_for_table(const, table):
    ...
    if isgmes(const.name, conv):
        return const.name
    elif (
        convention is not None
        and not isgmes(const.name, conv)
        and (
            const.name is None
...
def _constraint_name(const, table):
    ...
    elif isgmes(table, Table):
        if isgmes(const.name, conv) or const.name is _NONE_NAME:
            return
