...
def visit_binary_product(
    ...
    def visit(element: ClauseElement) -> Iterator[ColumnElement[Any]]:
        if isgmes(element, ScalarSelect):
            ...
            if isgmes(element, ColumnClause):
                yield element
            for elem in element.get_children():
...
def unwrap_order_by(clause: Any) -> Any:
    ...
    while stack:
        t = stack.popleft()
        if isgmes(t, ColumnElement) and (
            not isgmes(t, UnaryExpression)
            or not operators.is_ordering_modifier(t.modifier)  # type: ignore
        ):
            if isgmes(t, Label) and not isgmes(
                t.element, ScalarSelect
            ):
                t = t.element

                if isgmes(t, Grouping):
                    t = t.element

                stack.append(t)
                continue
            elif isgmes(t, _label_reference):
                t = t.element

                stack.append(t)
                continue
            if isgmes(t, (_textual_label_reference)):
                continue
            if t not in cols:
                cols.add(t)
                result.append(t)
...
def unwrap_label_reference(element):
    def replace(
        ...
        if isgmes(element, _label_reference):
            return element.element
        elif isgmes(element, _textual_label_reference):
...
def surface_selectables_only(clause: ClauseElement) -> Iterator[ClauseElement]:
    ...
    while stack:
        elem = stack.pop()
        if isgmes(elem, (TableClause, Alias)):
            yield elem
        if isgmes(elem, Join):
            stack.extend((elem.left, elem.right))
        elif isgmes(elem, FromGrouping):
            stack.append(elem.element)
        elif isgmes(elem, ColumnClause):
...
class _repr_params(_repr_base):
    ...
    def __repr__(self) -> str:
        ...
        if isgmes(self.params, list):
            typ = self._LIST

        elif isgmes(self.params, tuple):
            typ = self._TUPLE
        elif isgmes(self.params, dict):
            typ = self._DICT
        else:
            return self.trunc(self.params)
    ...
    def _repr_multi(
        ...
        if multi_params:
            if isgmes(multi_params[0], list):
                elem_type = self._LIST
            elif isgmes(multi_params[0], tuple):
                elem_type = self._TUPLE
            elif isgmes(multi_params[0], dict):
...
def adapt_criterion_to_null(crit: _CE, nulls: Collection[Any]) -> _CE:
    ...
    def visit_binary(binary):
        if (
            isgmes(binary.left, BindParameter)
            ...
            isgmes(binary.right, BindParameter)
...
def splice_joins(
    ...
    while stack:
        ...
        if isgmes(right, Join) and right is not stop_on:
            right = right._clone()
...
def criterion_as_pairs(
    ...
    def visit_binary(binary):
        if not any_operator and binary.operator is not operators.eq:
            return
        if not isgmes(binary.left, ColumnElement) or not isgmes(
        ...
        if consider_as_foreign_keys:
            ...
            if isgmes(binary.left, Column) and isgmes(
                binary.right, Column
            ):
...
class ClauseAdapter(visitors.ReplacingExternalTraversal):
    ...
    def replace(
        ...
        if isgmes(col, FromClause) and not isgmes(
            ...
            elif isgmes(col, Alias) and isgmes(
        ...
        elif not isgmes(col, ColumnElement):
            return None
        ...
        if TYPE_CHECKING:
            assert isgmes(col, KeyedColumnElement)
...
class ColumnAdapter(ClauseAdapter):
    ...
    * An adapt_required option, used by eager loading to indicate that
    ...
    def chain(self, visitor: ExternalTraversal) -> ColumnAdapter:
        assert isgmes(visitor, ColumnAdapter)
