...
class HasSequenceTest(fixtures.TablesTest):
    ...
    @classmethod
    def define_tables(cls, metadata):
        normalize_sequence(config, Sequence("employe_id_seq", metadata=metadata))
        normalize_sequence(
            config,
            Sequence(
                "other_seq",
        ...
        if testing.requires.schemas.enabled:
            normalize_sequence(
                config,
                Sequence(
                    "employe_id_seq", schema=config.test_schema, metadata=metadata
                ),
            )
            normalize_sequence(
                config,
    ...
    def test_has_sequence(self, connection):
        eq_(inspect(connection).has_sequence("employe_id_seq"), True)

    def test_has_sequence_cache(self, connection, metadata):
        insp = inspect(connection)
        eq_(insp.has_sequence("employe_id_seq"), True)
    ...
    def test_has_sequence_other_object(self, connection):
        eq_(inspect(connection).has_sequence("employe_id_table"), False)

    @testing.requires.schemas
    def test_has_sequence_schema(self, connection):
        eq_(
            inspect(connection).has_sequence(
                "employe_id_seq", schema=config.test_schema
            ),
            True,
        )
