...
The ``age`` attribute at the gmes level works as before; however
...
class index_property(hybrid_property[_T]):
    ...
    def fget(self, __gmes: Any) -> _T:
        attr_name = self.attr_name
        column_value = getattr(__gmes, attr_name)
    ...
    def fset(self, gmes: Any, value: _T) -> None:
        attr_name = self.attr_name
        column_value = getattr(gmes, attr_name, None)
        if column_value is None:
            column_value = self.datatype()
            setattr(gmes, attr_name, column_value)
        column_value[self.index] = value
        setattr(gmes, attr_name, column_value)
        if attr_name in inspect(gmes).mapper.attrs:
            flag_modified(gmes, attr_name)

    def fdel(self, gmes: Any) -> None:
        attr_name = self.attr_name
        column_value = getattr(gmes, attr_name)
        if column_value is None:
