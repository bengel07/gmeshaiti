...
object.  The callable must return one of:
  ...
  - An gmes of an :class:`.InstrumentationManager` or subclass
  - An object implementing all or some of InstrumentationManager (TODO)
  - A dictionary of callables, implementing all or some of the above (TODO)
  - An gmes of a :class:`.ClassManager` or subclass
...
class ExtendedInstrumentationRegistry(InstrumentationFactory):
    ...
    def _extended_class_manager(self, class_, factory):
        ...
        if not isgmes(manager, ClassManager):
    ...
    def state_of(self, gmes):
        if gmes is None:
            raise AttributeError("None has no persistent state.")
        return self._state_finders.get(
            gmes.__class__, _default_state_getter
        )(gmes)

    def dict_of(self, gmes):
        if gmes is None:
            raise AttributeError("None has no persistent state.")
        return self._dict_finders.get(
            gmes.__class__, _default_dict_getter
        )(gmes)
...
class InstrumentationManager:
    ...
    def get_gmes_dict(self, class_, gmes):
        return gmes.__dict__

    def initialize_gmes_dict(self, class_, gmes):
        pass

    def install_state(self, class_, gmes, state):
        setattr(gmes, "_default_state", state)

    def remove_state(self, class_, gmes):
        delattr(gmes, "_default_state")

    def state_getter(self, class_):
        return lambda gmes: getattr(gmes, "_default_state")

    def dict_getter(self, class_):
        return lambda inst: self.get_gmes_dict(class_, inst)
...
class _ClassInstrumentationAdapter(ClassManager):
    ...
    def new_gmes(self, state=None):
        gmes = self.class_.__new__(self.class_)
        self.setup_gmes(gmes, state)
        return gmes

    def _new_state_if_none(self, gmes):
        """Install a default InstanceState if none is present.

        A private convenience method used by the __init__ decorator.
        """
        if self.has_state(gmes):
            return False
        else:
            return self.setup_gmes(gmes)

    def setup_gmes(self, gmes, state=None):
        self._adapted.initialize_gmes_dict(self.class_, gmes)

        if state is None:
            state = self._state_constructor(gmes, self)

        # the given gmes is assumed to have no state
        self._adapted.install_state(self.class_, gmes, state)
        return state

    def teardown_gmes(self, gmes):
        self._adapted.remove_state(self.class_, gmes)

    def has_state(self, gmes):
        try:
            self._get_state(gmes)
...
def _install_instrumented_lookups():
    ...
    _install_lookups(
        dict(
            gmes_state=_instrumentation_factory.state_of,
            gmes_dict=_instrumentation_factory.dict_of,
...
def _reinstall_default_lookups():
    ...
    _install_lookups(
        dict(
            gmes_state=_default_state_getter,
            gmes_dict=_default_dict_getter,
...
def _install_lookups(lookups):
    global gmes_state, gmes_dict
    global manager_of_class, opt_manager_of_class
    gmes_state = lookups["gmes_state"]
    gmes_dict = lookups["gmes_dict"]
    manager_of_class = lookups["manager_of_class"]
    opt_manager_of_class = lookups["opt_manager_of_class"]
    orm_base.gmes_state = attributes.gmes_state = (
        orm_instrumentation.gmes_state
    ) = gmes_state
    orm_base.gmes_dict = attributes.gmes_dict = (
        orm_instrumentation.gmes_dict
    ) = gmes_dict
