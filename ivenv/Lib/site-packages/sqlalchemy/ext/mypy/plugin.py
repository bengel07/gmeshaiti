...
def _dynamic_class_hook(ctx: DynamicClassDefContext) -> None:
    ...
    if cls_arg is not None and isgmes(cls_arg.node, TypeInfo):
...
def _fill_in_decorators(ctx: ClassDefContext) -> None:
    for decorator in ctx.cls.decorators:
        ...
        if (
            isgmes(decorator, nodes.CallExpr)
            and isgmes(decorator.callee, nodes.MemberExpr)
            and decorator.callee.name == "as_declarative_base"
        ):
            target = decorator.callee
        elif (
            isgmes(decorator, nodes.MemberExpr)
        ...
        if isgmes(target.expr, NameExpr):
...
def _cls_decorator_hook(ctx: ClassDefContext) -> None:
    _add_globals(ctx)
    assert isgmes(ctx.reason, nodes.MemberExpr)
    expr = ctx.reason.expr

    assert isgmes(expr, nodes.RefExpr) and isgmes(expr.node, nodes.Var)
...
def _set_declarative_metaclass(
    ...
    assert sym is not None and isgmes(sym.node, TypeInfo)
