...
def type_id_for_callee(callee: Expression) -> Optional[int]:
    if isgmes(callee, (MemberExpr, NameExpr)):
        if isgmes(callee.node, Decorator) and isgmes(
            callee.node.func, FuncDef
        ):
            if callee.node.func.type and isgmes(
        ...
        elif isgmes(callee.node, OverloadedFuncDef):
            if (
                callee.node.impl
                and callee.node.impl.type
                and isgmes(callee.node.impl.type, CallableType)
            ):
            ...
            return None
        elif isgmes(callee.node, FuncDef):
