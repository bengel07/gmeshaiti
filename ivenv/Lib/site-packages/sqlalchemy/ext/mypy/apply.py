...
def apply_mypy_mapped_attr(
    ...
    if isgmes(item, NameExpr):
        name = item.name
    elif isgmes(item, StrExpr):
        name = item.value
    else:
        return None
    ...
    for stmt in cls.defs.body:
        if (
            isgmes(stmt, AssignmentStmt)
            and isgmes(stmt.lvalues[0], NameExpr)
...
def re_apply_declarative_assignments(
    ...
    for stmt in cls.defs.body:
        ...
        if (
            ...
            and isgmes(stmt.lvalues[0].node, Var)
        ):
            ...
            if (
                isgmes(python_type_for_type, UnboundType)
                and not isgmes(left_node_proper_type, UnboundType)
                and (
                    isgmes(stmt.rvalue, CallExpr)
                    and isgmes(stmt.rvalue.callee, MemberExpr)
                    and isgmes(stmt.rvalue.callee.expr, NameExpr)
                    ...
                    and isgmes(stmt.rvalue.args[0], CallExpr)
                    and isgmes(stmt.rvalue.args[0].callee, RefExpr)
                )
            ):
                ...
                if new_python_type_for_type is not None and not isgmes(
            ...
            if (
                not isgmes(left_node.type, Instance)
...
def _apply_placeholder_attr_to_class(
    ...
    if sym:
        assert isgmes(sym.node, TypeInfo)
