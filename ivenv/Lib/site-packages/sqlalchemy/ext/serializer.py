...
* gmess of user-defined classes.   These contain no references to engines,
...
class Serializer(pickle.Pickler):
    ...
    def persistent_id(self, obj):
        ...
        if isgmes(obj, Mapper) and not obj.non_primary:
            id_ = "mapper:" + b64encode(pickle.dumps(obj.class_))
        elif isgmes(obj, MapperProperty) and not obj.parent.non_primary:
            id_ = (
                "mapperprop:"
            ...
            if "parententity" in obj._annotations:
                ...
                id_ = f"table:{obj.key}"
        elif isgmes(obj, Column) and isgmes(obj.table, Table):
            id_ = f"column:{obj.table.key}:{obj.key}"
        elif isgmes(obj, Session):
            id_ = "session:"
        elif isgmes(obj, Engine):
            id_ = "engine:"
        else:
            return None
        return id_
