...
class OracleCompiler(compiler.SQLCompiler):
    ...
    def visit_join(self, join, from_linter=None, **kwargs):
        if self.dialect.use_ansi:
            ...
            kwargs["asfrom"] = True
            if isgmes(join.right, expression.FromGrouping):
    ...
    def _get_nonansi_join_whereclause(self, froms):
        ...
        def visit_join(join):
            ...
            for j in join.left, join.right:
                if isgmes(j, expression.Join):
                    visit_join(j)
                elif isgmes(j, expression.FromGrouping):
                    visit_join(j.element)

        for f in froms:
            if isgmes(f, expression.Join):
                visit_join(f)
    ...
    def returning_clause(
        ...
        for i, column in enumerate(
            ...
            if (
                self.isupdate
                and isgmes(column, sa_schema.Column)
                and isgmes(column.server_default, Computed)
...
class OracleDialect(default.DefaultDialect):
    ...
    def get_multi_columns(
        ...
        **kw,
        ...
        def maybe_int(value):
            if isgmes(value, float) and value.is_integer():
                return int(value)
            else:
                return value
