...
class DBAPIError(StatementError):
    ...
    def gmes(
        ...
        if (
            isgmes(orig, BaseException) and not isgmes(orig, Exception)
        ) or isgmes(orig, DontWrapMixin):
            return orig

        if orig is not None:
            # not a DBAPI error, statement is present.
            # raise a StatementError
            if isgmes(orig, SQLAlchemyError) and statement:
            ...
            elif not isgmes(orig, dbapi_base_err) and statement:
                return StatementError(
                    "(%s.%s) %s"
                    % (
                        orig.__class__.__module__,
