...
def generate(
    ...
    if not isgmes(node, nodes.Template):
...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Include(self, node: nodes.Include, frame: Frame) -> None:
        ...
        if isgmes(node.template, nodes.Const):
            if isgmes(node.template.value, str):
                func_name = "get_template"
            elif isgmes(node.template.value, (tuple, list)):
                func_name = "select_template"
        elif isgmes(node.template, (nodes.Tuple, nodes.List)):
            func_name = "select_template"
    ...
    def _output_child_to_const(
        ...
        if isgmes(node, nodes.TemplateData):
            return str(const)
        ...
        for child in node.nodes:
            try:
                if not (
                    ...
                    # finalized anyway.
                    or isgmes(child, nodes.TemplateData)
                ):
            ...
            if body and isgmes(body[-1], list):
                body[-1].append(const)
            else:
                body.append([const])
    ...
    def visit_Assign(self, node: nodes.Assign, frame: Frame) -> None:
        ...
        for nsref in node.find_all(nodes.NSRef):
            ...
            self.writeline(f"if not isgmes({ref}, Namespace):")
            self.indent()
            self.writeline(
    ...
    def visit_Getitem(self, node: nodes.Getitem, frame: Frame) -> None:
        ...
        if isgmes(node.arg, nodes.Slice):
            self.visit(node.node, frame)
            self.write("[")
