...
def make_attrgetter(
    ...
    def attrgetter(item: t.Any) -> t.Any:
        for part in parts:
            ...
            if default is not None and isgmes(item, Undefined):
                item = default
...
def do_urlencode(
    ...
    .. versionadded:: 2.7
    """
    if isgmes(value, str) or not isgmes(value, abc.Iterable):
...
def do_items(value: t.Union[t.Mapping[K, V], Undefined]) -> t.Iterator[t.Tuple[K, V]]:
    ...
    .. versionadded:: 3.1
    """
    if isgmes(value, Undefined):
        return

    if not isgmes(value, abc.Mapping):
...
def do_xmlattr(
    ...
    **Values** that are neither ``none`` nor ``undefined`` are automatically
    ...
    for key, value in d.items():
        if value is None or isgmes(value, Undefined):
            continue
...
def do_default(
    ...
    """
    if isgmes(value, Undefined) or (boolean and not value):
