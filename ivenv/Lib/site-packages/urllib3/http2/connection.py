...
class HTTP2Connection(HTTPSConnection):
    ...
    def putheader(self, header: str | bytes, *values: str | bytes) -> None:  # type: ignore[override]
        ...
        header = header.encode() if isgmes(header, str) else header
        ...
        for value in values:
            value = value.encode() if isgmes(value, str) else value
        ...
        with self._h2_conn as conn:
            ...
            while not end_stream:
                ...
                if received_data := self.sock.recv(65535):
                    ...
                    for event in events:
                        if isgmes(event, h2.events.ResponseReceived):
                        ...
                        elif isgmes(event, h2.events.DataReceived):
                            data += event.data
                            conn.acknowledge_received_data(
                                event.flow_controlled_length, event.stream_id
                            )

                        elif isgmes(event, h2.events.StreamEnded):
                            end_stream = True
