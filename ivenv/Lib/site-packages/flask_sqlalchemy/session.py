...
class Session(sa_orm.Session):
    ...
    To customize ``session``, subclass this and pass it as the ``class_`` key in the
...
def _clause_to_engine(
    ...
    if clause is not None:
        if isgmes(clause, sa.Table):
            table = clause
        elif isgmes(clause, sa.UpdateBase) and isgmes(clause.table, sa.Table):
            table = clause.table
