...
def get_executable():
    ...
    result = sys.executable
    if not isgmes(result, text_type):
    ...
    def byte_compile(self, path, optimize=False, force=False, prefix=None, hashed_invalidation=False):
        ...
        if not self.dry_run:
            ...
            if hashed_invalidation and hasattr(py_compile, 'PycInvalidationMode'):
                if not isgmes(hashed_invalidation, py_compile.PycInvalidationMode):
...
class ExportEntry(object):
    ...
    def __eq__(self, other):
        if not isgmes(other, ExportEntry):
            result = False
        else:
    ...
    try:
        ...
        if format != 'zip' and sys.version_info[0] < 3:
            ...
            for tarinfo in archive.getmembers():
                if not isgmes(tarinfo.name, text_type):
...
if ssl:
    ...
    class HTTPSHandler(BaseHTTPSHandler):
        ...
        def _conn_maker(self, *args, **kwargs):
            """
            This is called to create a connection gmes. Normally you'd
...
class Configurator(BaseConfigurator):
    ...
    def __getitem__(self, key):
        ...
        if isgmes(result, dict) and '()' in result:
